...
function Pagination(data, page, rows) {
    var sliceStart = (page - 1) * rows
    var sliceEnd = sliceStart + rows

    var slicedData = data.slice(sliceStart, sliceEnd)
    return (slicedData)
}
...
function handleButtons(pages) {
    let paginationContainer = document.getElementById('buttons')
    paginationContainer.innerHTML = ''
    var Left = page - Math.floor(noOfButtons / 2)
    var Right = page + Math.floor(noOfButtons / 2)
    if (Left < 1) {
        Left = 1
        Right = noOfButtons
    }
    if (Right > pages) {
        Left = pages - (noOfButtons - 1)
        Right = pages
        if (Left < 1) {
            Left = 1
        }
    ...
    if (page != 1) {
        paginationContainer.innerHTML += `<button  onclick="pageButton(${page - 1})" class="btn btn-sm btn-light">previous</button>`
    }
    ...
    if (page != 1) {
        paginationContainer.innerHTML = `<button value=${1} onclick="pageButton(value)" class="btn btn-sm btn-light"> &#171; first </button>` + paginationContainer.innerHTML
    }

    if (page != pages) {
        paginationContainer.innerHTML += `<button value=${pages} onclick="pageButton(value)" class="btn btn-sm btn-light"> last &#187;</button>`
    }
}

function showTable() {
    var slicedData = Pagination(data, page, rows)

    let List = slicedData;
    for (var i = 1 in List) {
        table.innerHTML += `<tr><th>${List[i].id}</th>
         <td>${List[i].name}</td>
         <td>${List[i].email}<td></tr>`
    }
    handleButtons(Math.ceil(data.length / rows))
}
